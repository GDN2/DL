...
for epoch in range(epochs):
    for index in range(len(loaded_data)):
        ...
        target_data = np.zeros([1,output_nodes]) + delta_x
...
#plt.show() # plt.show()는 맨 마지막에 안하면 도중에 이미지 보여주는 루프를 돌기 때문에 창을 끄기 전까지 멈춤 jupyter 추천
plt.grid(False) #원래는 loss_val추세를 보기 위해서 plt.show()를 위해서 나둬야 했는데 이후의 image 저장 코드를 위해서 빼둠
...
for i in range(len(unmatched_list)):
    img = x_data[unmatched_list[i][0], :].reshape(28, 28)
    ...
    # plt.show() plt.show()하면 pycharm에서는 멈춤 jupyter추천
end = datetime.now()
print("Time", end-start)
print("All images are saved by png images")
os.chdir(cdir)
print(os.getcwd())