...
str = open('texts/text3.txt', 'rb').read().decode('latin-1')

cv.namedWindow('img_result') #threshold를 위해 img받을 창 생성
cv.createTrackbar('threshold', 'img_result', 0, 255, nothing)
#threshold생성 값에 따른 결과를 바로보여주며 최적화할 때 용이
#nothing은 리턴함수를 주어야하는데 여기서는 아무것도 없는 void함수줌
cv.setTrackbarPos('threshold', 'img_result', 0)
#최적화시 초기값 threshold줌

cap = cv.VideoCapture(0) #WepCam에서 영상 받음

while(True):

    #img_color = cv.imread('imgs/notebook.jpg')
    #img_color2 = cv.imread('imgs/1.png', cv.IMREAD_COLOR)
    ...
    for idx, centroid in enumerate(centroids):
        ...
        if area > 500:
            ...
            cv.rectangle(img_color, (x,y), (x, y+height), (0,0,255))
            y0, dy = y, 15
            for i, line in enumerate(str.split('\n')): #+는 아래로 -는 위로
                y = y0 + i*dy
                cv.putText(img_color, line, (x, y), cv.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 0))
