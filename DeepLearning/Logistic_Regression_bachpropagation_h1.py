...
class LogisticRegression:
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate=1e-2):
        ...
        self.Z3 = np.zeros([1, output_nodes])  # a3에다가 붙이면 안됨 y값이 1보다 커지고 log에 -들어가서 에러
...
train_loaded_data = np.loadtxt('./data/mnist_train_normV2.csv', delimiter=',', dtype=np.float32)
...
start_time = datetime.now()
test_loaded_data = np.loadtxt('./data/mnist_test_normV2.csv', delimiter=',', dtype=np.float32)
...
obj = LogisticRegression(input_nodes, hidden_nodes, output_nodes, 0.5)
start_time = datetime.now()
...
obj = LogisticRegression(input_nodes, hidden_nodes, output_nodes, 1e-2)
start_time = datetime.now()
for epoch in range(epochs):
    for index in range(len(train_data)):
        input_data = (train_data[index, :])
    ...
    if epoch % 10 == 0:
        print("epoch =", epoch, "error_val =", obj.error_val())
end_time = datetime.now()
print("Time spend ==>", end_time - start_time)
